<template>
  <a-card class="w-100" :title="$t('customer_list')">
    <template slot="extra">
      <router-link
    ...
    <div class="table-responsive">
      ...
      >
        ...
        <div slot="action" slot-scope="text, record">
          <router-link :to="{ name: 'customer-id', params: { id: record.id } }">
            {{ $t("edit") }}
          </router-link>
  ...
  methods: {
    ...
    onRemove(id) {
      this.$confirm({
        ...
        zIndex: 10002,
        onOk: () => {
          // this.$axios.$delete("customer/" + id).then(() => {
          //   this.fetch();
          // });
        },
        onCancel: () => {},
