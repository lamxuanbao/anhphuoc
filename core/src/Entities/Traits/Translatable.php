...
trait Translatable
{
            ...
            if ($attribute === 'translations') {
                foreach ($values as $locale => $item) {
                    if (in_array($locale, $locales)) {
                        $this->getTranslationOrNew($locale)->fill($item);
                    }
                }
            } else {
                if ($this->isTranslationAttribute($attribute)) {
    ...
    public function scopeOrWhereTranslationLike(
        Builder $query,
    ...
    public function scopeOrderByTranslation(Builder $query, string $translationField, string $sortMethod = 'asc')
    {
        ...
        return $query
            ->join($translationTable,
                function (JoinClause $join) use ($translationTable, $localeKey, $table, $keyName) {
                    $join
